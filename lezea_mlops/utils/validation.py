...
class LeZeAValidator:
    ...
    def validate_experiment_name(cls, name: str) -> bool:
        ...
        if len(name) > cls.MAX_EXPERIMENT_NAME_LENGTH:
            raise ValidationError(f"Experiment name too long: {len(name)} > {cls.MAX_EXPERIMENT_NAME_LENGTH}")
        
        if not re.match(cls.EXPERIMENT_NAME_PATTERN, name):
            raise ValidationError(f"Invalid experiment name format: {name}. Use only letters, numbers, underscore, hyphen")
    ...
    def validate_model_params(cls, params: Dict[str, Any]) -> bool:
        ...
        for key, value in params.items():
            ...
            if len(key) > cls.MAX_PARAM_NAME_LENGTH:
                raise ValidationError(f"Parameter name too long: {len(key)} > {cls.MAX_PARAM_NAME_LENGTH}")
    ...
    def _validate_param_value(cls, name: str, value: Any) -> None:
        ...
        if isinstance(value, (int, float)) and name in numeric_ranges:
            ...
            if not (min_val <= value <= max_val):
                raise ValidationError(f"Parameter '{name}' value {value} outside valid range [{min_val}, {max_val}]")
    ...
    def validate_metrics(cls, metrics: Dict[str, Union[float, int]]) -> bool:
        ...
        for name, value in metrics.items():
            ...
            if len(name) > cls.MAX_METRIC_NAME_LENGTH:
                raise ValidationError(f"Metric name too long: {len(name)} > {cls.MAX_METRIC_NAME_LENGTH}")
            ...
            if not isinstance(value, (int, float)):
                raise ValidationError(f"Metric '{name}' value must be numeric, got {type(value)}: {value}")
            ...
            if isinstance(value, float):
                if not (value == value):  # NaN check
                    raise ValidationError(f"Metric '{name}' is NaN")
                if abs(value) == float('inf'):
                    raise ValidationError(f"Metric '{name}' is infinite")
    ...
    def validate_file_path(cls, path: Union[str, Path], must_exist: bool = True, 
                          check_permissions: bool = True) -> bool:
        """
            ...
            if path.is_file():
                ...
                if path.suffix in ['.yml', '.yaml', '.json']:
                    if size > cls.MAX_CONFIG_FILE_SIZE:
                        raise ValidationError(f"Config file too large: {size} > {cls.MAX_CONFIG_FILE_SIZE}")
                
                elif path.suffix == '.log':
                    if size > cls.MAX_LOG_FILE_SIZE:
                        raise ValidationError(f"Log file too large: {size} > {cls.MAX_LOG_FILE_SIZE}")
    ...
    def validate_dataset_info(cls, dataset_info: Dict[str, Any]) -> bool:
        ...
        if not isinstance(version, str) or not re.match(r'^\d+\.\d+\.\d+$', version):
            raise ValidationError(f"Dataset version must be semantic version (X.Y.Z), got: {version}")
        ...
        if format_type not in valid_formats:
            raise ValidationError(f"Invalid dataset format: {format_type}. Valid: {valid_formats}")
